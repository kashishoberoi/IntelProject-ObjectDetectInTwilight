...
The way in which YOLO works is that an image is given as an input and laters it splitted into a square grid say SxS, within each of the grid m bounding boxes are taken. A class probability and offset values for each bounding box are send as an output by the network and boxes are used to locate the object within the image having the class probability above a threshold value. Here is an architecture of YOLOv3 model.<br/>
