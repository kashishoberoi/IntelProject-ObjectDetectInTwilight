...
The input to the network is an image of dimensions 224x224 RGB(i.e. 224x224x3) image. Afterwards the image is being passed through a series of convolutional layers where the filters have a size of 3x3 also having the same padding. There is a padding of 1-pixel (same padding) done after each convolution layer to prevent the spatial feature of the image. After passing the images through a series of convolutional layers and max-pooling layer, what we get is a feature map of dimension 7x7x512, which in-turn is flattened to obtain a feature map of dimension 1x25088. Proceeding to this are the 3 fully connnected layers, where first & second layers takes input from the last feature vector and outputs a vector of dimension 1x4096 and the third layer outputs 1000 channeels for 1000 classes, which is then passed to softmax layer in order to normalize the classification vector.<br/>
### R-CNN
